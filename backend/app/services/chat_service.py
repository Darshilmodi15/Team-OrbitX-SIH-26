...
class ChatStorageService:
    ...
    def serialize(self, row: Conversation) -> ConversationSummary:
        return ConversationSummary(id=row.id, title=row.title, created_at=row.created_at, updated_at=row.updated_at, messages=[StoredChatMessage(id=m.id, role=m.role, content=m.message, created_at=m.created_at, metadata=json.loads(m.sources_used_json) if m.sources_used_json else None) for m in row.messages])
