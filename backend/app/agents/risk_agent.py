...
def assess_risk(weather_input: Union[WeatherEvidence, Dict[str, Any]]) -> RiskEvidence:
    ...
    fields = ("wave_height_m", "wind_speed_kmh", "wave_period_s", "wind_gust_kmh", "visibility_km", "weather_code")
    available = {}
    for key in fields:
        value = raw.get(key)
    ...
    tiers = [wave.level, wind.level, gust.level, storm.level]
    level = "unsafe" if "HIGH" in tiers else "caution" if "MODERATE" in tiers else "unknown" if missing else "low"
    trend = "unknown"
    ...
    if usable and "wave_height_m" in available and len(horizon) >= 2:
        ...
        if all(isinstance(v, (int, float)) and not isinstance(v, bool) and math.isfinite(v) and v >= 0 for v in waves):
            ...
            if trend == "deteriorating" and level != "unsafe":
                level = "caution"
    warnings = [item.description for item in (wave, wind, gust, storm) if item.level in {"HIGH", "MODERATE"}]
