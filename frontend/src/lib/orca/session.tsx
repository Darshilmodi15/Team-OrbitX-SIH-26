...
import { getUserProfile, fetchSavedLocation, loginUser, registerUser, setAuthFailureHandler } from "@/services/api";
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const signOut = useCallback(() => { setUser(null); setToken(null); setLocationState(null); sessionStorage.removeItem(SESSION_TOKEN_KEY); localStorage.removeItem("orca.user"); localStorage.removeItem("orca.auth.token"); localStorage.removeItem("orca.location"); localStorage.removeItem("orca.chat.threads.v2"); localStorage.removeItem("orca_assistant_threads_v1"); }, []);
  ...
  useEffect(() => {
    ...
    fetchSavedLocation().then(saved => {
      if (!cancelled && saved?.is_coastal_supported) setLocationState({ coords: {lat: saved.lat, lon: saved.lon}, label: `${saved.lat.toFixed(4)}, ${saved.lon.toFixed(4)}`, admin: saved.coastal_region, distanceToCoastKm: saved.distance_to_coast_km, area: "coastal", source: "manual" });
    }).catch(() => { /* Leave location unavailable; never invent a city. */ });
