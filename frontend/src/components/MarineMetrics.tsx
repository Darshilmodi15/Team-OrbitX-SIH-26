    ...
    <div className="mt-3 p-3 rounded-xl bg-slate-900/70 border border-slate-800 text-xs">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        ...
        <div className="p-2 rounded-lg bg-slate-950/60 border border-slate-800/80">
          ...
          <div className={`text-sm font-semibold mt-0.5 ${wave > 2.0 ? 'text-rose-400' : wave > 1.5 ? 'text-amber-400' : 'text-emerald-400'}`}>
            {wave} <span className="text-xs font-normal text-slate-400">m</span>
          </div>
        </div>

        <div className="p-2 rounded-lg bg-slate-950/60 border border-slate-800/80">
          <div className="text-[10px] text-slate-400 uppercase font-mono">Wind Speed</div>
          <div className={`text-sm font-semibold mt-0.5 ${wind > 45 ? 'text-rose-400' : wind > 35 ? 'text-amber-400' : 'text-cyan-400'}`}>
            {wind} <span className="text-xs font-normal text-slate-400">km/h</span>
          </div>
        </div>
