...
interface Props {isOpen: boolean; onClose:()=>void; userLocation:LocationCoords; currentLang?:string;}
export default function EmergencySOSModal({isOpen,onClose,userLocation}:Props) {
 const {t,lang}=useI18n(); const old=getStrings(lang);
 ...
 const [record,setRecord]=useState<{sos_id:string}|null>(null); const [notes,setNotes]=useState(""); const [saved,setSaved]=useState(false);
 async function send() {
  if(pending.current) return; pending.current=true; setBusy(true); setFailed(false);
  try { const result=await broadcastSOS({lat:userLocation.lat,lon:userLocation.lon,emergency_nature:"General Emergency"});
   if(!result?.sos_id || result.status!=="RECEIVED") throw new Error("SOS_NOT_CONFIRMED"); setRecord(result);
  } catch {setFailed(true);} finally {pending.current=false;setBusy(false);}
 }
 ...
 return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="sos-title" className="max-h-[90dvh] w-full max-w-lg overflow-auto rounded-lg border border-border bg-card p-5 text-card-foreground">
  ...
  <p className="mb-4 text-sm">{t("loc.current")}: {userLocation.lat.toFixed(4)}, {userLocation.lon.toFixed(4)}</p>
  {failed && <p role="alert" className="my-3 text-destructive">{t("state.liveUnavailable")}</p>}
  {!record ? <button className="min-h-12 w-full rounded-md bg-destructive px-4 text-white disabled:opacity-50" onClick={send} disabled={busy}>{busy?t("state.loading"):t("sos.submit")}</button> : <>
   ...
   <label className="mt-4 block">{old.sosDistressNature}<textarea className="mt-2 min-h-32 w-full rounded-md border border-border bg-background p-3" value={notes} maxLength={2000} onChange={e=>{setNotes(e.target.value);setSaved(false);}} /></label>
