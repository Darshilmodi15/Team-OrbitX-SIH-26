...
export default function Navbar({
  ...
  const currentBadge = safetyBadges[riskLevel] || safetyBadges.safe;
  const SafetyIcon = currentBadge.icon;
    ...
    <header className="sticky top-0 z-40 w-full border-b border-slate-200 bg-white/95 backdrop-blur-md shadow-xs">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-3 sm:px-6">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          {/* Location Badge (Clickable to change port) */}
          <div className="hidden md:flex items-center">
            <div className="h-4 w-px bg-slate-200 mx-2" />
            <button
              type="button"
              onClick={onOpenLocation}
              className="group flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50/80 px-2.5 py-1 text-xs text-slate-700 hover:border-[#0D9488] hover:bg-teal-50/50 transition cursor-pointer"
            ...
            >
              ...
              <span className="text-[10px] text-slate-500 font-mono">
                ({coastInfo.distanceKm.toFixed(1)} km)
              </span>
          ...
          {currentUser ? (
            <div className="relative" ref={userMenuRef}>
              <button
                ...
                className="flex h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-800 hover:bg-slate-50 transition cursor-pointer"
              >
              ...
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-1.5 z-50 w-56 rounded-xl border border-slate-200 bg-white p-1 shadow-xl animate-scaleIn">
                  <div className="px-3 py-2 border-b border-slate-100">
                    <p className="text-xs font-bold text-slate-900 truncate">{currentUser.name}</p>
                    <p className="text-[11px] text-slate-500 truncate">{currentUser.email || currentUser.mobile_number}</p>
                    <span className="mt-1 inline-block rounded-md bg-slate-100 px-1.5 py-0.5 text-[9px] font-mono font-bold text-slate-600">
                      {currentUser.role}
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs text-red-600 hover:bg-red-50 transition cursor-pointer border-t border-slate-100 mt-1"
                  >
            ...
            <button
              type="button"
              onClick={onOpenAuth}
              className="flex h-8 items-center gap-1.5 rounded-lg bg-[#0A2540] px-3 text-xs font-bold text-white hover:bg-[#081D33] active:scale-95 transition cursor-pointer"
            >
