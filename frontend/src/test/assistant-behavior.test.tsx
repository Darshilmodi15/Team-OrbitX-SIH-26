...
vi.mock("@/components/orca/AppShell", () => ({
  AppShell: ({ children }: any) => <>{children}</>,
}));
vi.mock("@/lib/orca/voice-audio", () => ({
  speechWav: vi.fn(async (audio: Blob) => audio),
}));
vi.mock("@/components/SEO", () => ({ SEO: () => null }));
vi.mock("@/lib/orca/session", () => ({
  useSession: () => ({
    user: { id: "u1" },
    location: { coords: { lat: 20.9, lon: 70.3 } },
  }),
}));
...
vi.mock("@/lib/orca/snapshot", async (original) => ({
  ...
  useMarineSnapshot: () => snapshotMock,
}));
import { fetchSnapshotById } from "@/lib/orca/snapshot";
    ...
    <MemoryRouter initialEntries={[path]}>
      <I18nProvider>
        <Routes>
          ...
          <Route
            path="/assistant/c/:conversationId"
...
it("typed enter submits the actual message once", async () => {
  ...
  const input = screen.getByPlaceholderText(
    "Ask anything about your coastal safety…",
  );
  fireEvent.change(input, { target: { value: "Explain waves" } });
  fireEvent.keyDown(input, { key: "Enter", shiftKey: false });
  await waitFor(() => expect(sendChatMessage).toHaveBeenCalledTimes(1));
  expect(sendChatMessage).toHaveBeenCalledWith(
    expect.objectContaining({ message: "Explain waves" }),
  );
});
...
it("provider failure is an error, not an assistant message", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "What does PFZ mean?" }));
  expect(await screen.findByRole("alert")).toHaveTextContent(
    "ORCA’s AI provider is temporarily unavailable",
  );
...
it("keeps unrelated server errors generic without exposing technical details", async () => {
  vi.mocked(sendChatMessage).mockRejectedValueOnce(
    new Error("Internal database exception"),
  );
  mount();
  fireEvent.click(screen.getByRole("button", { name: "What does PFZ mean?" }));
  expect(await screen.findByRole("alert")).toHaveTextContent(
  ...
  expect(
    screen.queryByText("Internal database exception"),
...
it("reload of a failed request retries the persisted request ID without a duplicate user bubble", async () => {
    ...
    messages: [
      {
        id: "server-user",
        role: "user",
        content: "Explain waves",
        created_at: new Date().toISOString(),
        metadata: {
          request_id: "durable-attempt",
          request_language: "en",
          requested_time: null,
        },
      },
  ...
  await waitFor(() =>
    expect(
      screen.getByPlaceholderText("Ask anything about your coastal safety…"),
    ).toHaveValue("Explain waves"),
  );
  fireEvent.keyDown(
    screen.getByPlaceholderText("Ask anything about your coastal safety…"),
    { key: "Enter" },
...
it("voice yields an editable transcript and submits only through the normal text pipeline", async () => {
  ...
  vi.mocked(transcribeVoiceAudio).mockResolvedValue({
    transcript: "Test spoken question",
    language: "en",
    english_transcript: "Test spoken question",
    is_mock: false,
  ...
  expect(screen.getByText(/Transcript ready/)).toBeVisible();
  expect(sendChatMessage).not.toHaveBeenCalled();
  fireEvent.change(input, { target: { value: "Edited spoken question" } });
  fireEvent.keyDown(input, { key: "Enter" });
  await waitFor(() =>
    expect(sendChatMessage).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Edited spoken question" }),
    ),
  );
...
it("map prompt remains editable and cannot send while its snapshot is unresolved", async () => {
  vi.mocked(fetchSnapshotById).mockReturnValue(new Promise(() => {}));
  mount("/assistant?snapshot=pending&prompt=Explain%20these%20conditions");
  const input = screen.getByPlaceholderText(
    "Ask anything about your coastal safety…",
  );
  await waitFor(() => expect(input).toHaveValue("Explain these conditions"));
  fireEvent.keyDown(input, { key: "Enter" });
  expect(sendChatMessage).not.toHaveBeenCalled();
});
it("rejects map context from a different saved location", async () => {
  vi.mocked(fetchSnapshotById).mockResolvedValue({
    snapshot_id: "foreign-place",
    location: { lat: 18.9, lon: 72.7 },
  } as any);
  mount(
    "/assistant?snapshot=foreign-place&prompt=Explain%20these%20conditions",
  );
  await waitFor(() =>
